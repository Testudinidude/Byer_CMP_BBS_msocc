---
title: "Breeding Bird Surveys - Cleveland Metroparks"
...
CMP_bbs_pcap_join_pcaponly_pcapsum<-read.csv("CMP_bbs_summary.csv")

birdcomm<-CMP_bbs_pcap_join_pcaponly_pcapsum[,31:(133)]
...
```{r,warning=FALSE,error=FALSE,message=FALSE}
vegecomm<-CMP_bbs_pcap_join_pcaponly_pcapsum[,c(6:15,18:26)]
...
```{r,warning=FALSE,error=FALSE,message=FALSE}
landcomm<-CMP_bbs_pcap_join_pcaponly_pcapsum[,c(16,17,27,28,29,150,159,160)]
...
ggord(p_landveg_step,  xlim=c(-1.25,1.25),ylim = c(-1, 0.75),txt=4,arrow=TRUE,ptslab=F,addsize=4, size =NA,addcol = color_resid,labcol=NA)

ggord(p_landveg_step,txt=4,arrow=TRUE,ptslab=F,addsize=NA, size =4,addcol = color_resid,labcol=NA)
...
CMP_bbs_pcap_join<-read.csv("CMP_bbs_obsdata.csv",header=T)
...
landcomm$simp_plot<-CMP_bbs_pcap_join_pcaponly_pcapsum$simp_plot

class(CMP_bbs_pcap_join_pcaponly$simp_plot)<-"character"

class(landcomm$simp_plot)<-"character"
...
write_rds(bbs_occobject_three,"bbs_occobject_three.rds")
...
knitr::kable(AICc_table[order(AICc_table$wAIC),])
```
...
We can then read in some of those simplified datasets for plotting.
...
We can generate plots of predicted occupancy for these 164 plots. NOTE - given that the provided data do not have coordinates, this will produce unsatisfying outcomes with the coordinate-stripped datasets. This general approach, however, will work for interpolation of occupancy estimates. 

```{r, echo = FALSE, eval = FALSE}
fitted.ms.fulllandveg<-fitted(out.ms.landvegrda_2)
...
While the below RShiny app should be fairly easy to use within the current markdown, you may also access this via: <https://testudinidude.shinyapps.io/bbsapp/>.
...
```{r}
covars_comm<-read.csv("comm_plotting.csv")

covars_comm_occ<-covars_comm[covars_comm$Process %in% "Occupancy",]

covars_comm_occ$Covariate<-factor(covars_comm_occ$Covariate,levels=c("dicot","monocots","canopy_dens","carex","perc_open","svp","tol_rel","developed.x","ldi"))

covars_comm_occ<-covars_comm_occ[order(covars_comm_occ$Covariate,decreasing = T),]


ggplot(aes(x=Mean,y=factor(Covariate,levels=Covariate)),data=covars_comm_occ)+geom_point(size=3)+geom_errorbarh(aes(xmin=X5.,xmax=X95.))+theme_classic(base_size=16)+xlab("Covariate estimate")+ylab("Covariates") + geom_vline(xintercept=0)

ggplot(aes(x=Mean,y=factor(Covariate,levels=Covariate)),data=covars_comm_occ)+geom_point(size=3)+geom_errorbarh(aes(xmin=X5.,xmax=X95.))+theme_classic(base_size=16)+xlab("Covariate estimate")+ylab("Covariates") + geom_vline(xintercept=0)+geom_point(aes(x=Mean_S),color="#009E73",size=3)+ geom_point(aes(x=Mean_T),color="#56B4E9",size=3)+geom_point(aes(x=Mean_R),color="#E69F00" ,size=3)

ggplot(aes(x=Mean,y=factor(Covariate,levels=Covariate)),data=covars_comm)+theme_classic(base_size=16)+xlab("Covariate estimate")+ylab("Covariates") + geom_vline(xintercept=0)+geom_point(aes(x=Mean_S),color="#009E73",size=3)+ geom_errorbarh(aes(xmin=X5._S,xmax=X95._S),color="#009E73")+
geom_point(aes(x=Mean_T),color="#56B4E9",size=3)+geom_errorbarh(aes(xmin=X5._T,xmax=X95._T),color="#56B4E9")+geom_point(aes(x=Mean_R),color="#E69F00" ,size=3)  + geom_errorbarh(aes(xmin=X5._R,xmax=X95._R),color="#E69F00")

```

```{r,fig.height=12,warning=FALSE,echo=FALSE,message=FALSE}
covars<-read.csv("sppplotting.csv",header=T)
...
covars_nodet_sigspp_summary<-covars_nodet_sigspp %>%
  dplyr::select(Covariate,Mean,Migrant.Status) %>%
  group_by(Covariate,Migrant.Status) %>%
  summarise(MeanEffect=mean(Mean))

covars_nodet<-covars[covars$Process == "Occupancy",]


covars_nodet$Covariate<-factor(covars_nodet$Covariate,levels=c("ldi","developed.x","tol_rel","svp","perc_open","carex","canopy_dens","monocots","dicot"))

covars_nodet<-covars_nodet[order(covars_nodet$Covariate,decreasing = F),]


ggplot(aes(x=Mean,y=Covariate,color=Migratory),data=covars_nodet)+geom_boxplot(position="dodge")+scale_color_manual(values=c("#E69F00" ,"#009E73","#56B4E9")) +geom_vline(xintercept = 0) + theme_classic(base_size=16)+scale_y_discrete(c("dicot","monocots","canopy_dens","carex","perc_open","svp","tol_rel","developed.x","ldi"))+theme(axis.text.y = element_text(size = 16,colour="black"),axis.text.x = element_text(size = 16,colour="black"),axis.title.x=element_text(size=20,colour="black"),axis.title.y=element_text(size=16,colour="black"),legend.position="none")+xlab("Mean covariate estimate")
